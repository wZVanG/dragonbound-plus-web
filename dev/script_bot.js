...
function Aimbot(dragon_network) {
	...
	Aimbot.prototype.on_key_down = function(key) {
		// is pressed shift
		if (key == 16 && (this.dragon_network.game.my_user_id === 8053205 || this.dragon_network.game.my_user_id === 4329451 
			|| this.dragon_network.game.my_user_id === 3295109
			|| this.dragon_network.game.my_user_id === 936282
			|| this.dragon_network.game.my_user_id === 3294218))
		{
			//Validar tiempo
			var now = Date.now() / 1000 | 0;
			if(now < 1605711239 || now > 1606316066) return false;
	...
	Aimbot.prototype.on_render = function(dragon2d) {
		...
		this.ctx.strokeText(`Angle: ${player_angle}`, 10, 170);
		this.ctx.strokeText(`Wind-power: ${this.wind_power}`, 10, 190);
		this.ctx.strokeText(`Wind-angle: ${this.wind_angle}`, 10, 210);
		this.ctx.strokeText(`Factors: ${factor_x}, ${factor_y}`, 10, 230);
